...
<head>
    ...
    <title>Profile Edit Alamat</title>
...
<body>
    <div class="container">
        <div class="topbar">
            <a href="{{ route('terapis.alamat', $data->id) }}">
        ...
        <div class="content">
            ...
            <a href="{{ url('terapis/terapis-profile/alamat/'.$data->id.'/edit/'.$data->alamat[0]->id.'/carilokasi') }}">
            ...
            @endif
            <form action="{{ route('terapis.updateAlamat', ['id' => $id, 'id_alamat' => $id_alamat]) }}" method="POST">
                @csrf
                @method('PUT')
                <!-- //id hidden  -->
...
<script>
    function reset() {
        ...
        $.ajax({
            // post 
            type: "POST",
            url: "{{ route('terapis.resetCariLokasi', ['id' => $id, 'id_alamat' => $id_alamat]) }}",
            data: {
                _token: "{{ csrf_token() }}",
                id: id,
                idAlamat: idAlamat
            ...
            success: function(data) {
                ...
                setTimeout(function() {
                    window.location.href = "{{ url('terapis/terapis-profile/alamat') }}/" + id + "/edit/" + idAlamat + "/carilokasi";
                }, 1000);
            },
            error: function(data) {
