...
<body>
    <div class="container">
        ...
        <div class="identitas">
            ...
            <table class="tableidentitas">
                <tr style="color: white; ">
                    <td style="font-size:14px; padding-bottom: 10px;" align="left"><b>Pekerjaan Diterima</b></td>
                    <td style="font-size:14px; padding-bottom: 10px;" align="right" id="durasi"><b style="font-size:18px;">
            ...
            <table class="table1">
                ...
                <tr>
                    <td colspan="3" align="left">
                        {{ $alamat->alamat_lengkap }}, Kelurahan. {{ $alamat->kelurahan }}, Kecamatan. {{ $alamat->kecamatan }}, {{ $alamat->kota }}, Provinsi. {{ $alamat->provinsi }}, Kode Pos ( {{ $alamat->kode_pos }} )
                    </td>
                </tr>
                <!-- LAYANAN -->
                <tr>
                ...
                <tr>
                    ...
                    <td width="200px" align="right" colspan="2" class="text-success">
                        @if($pemesanan->status_pemesanan == 'Suskes')
                        Lunas
        ...
        <div class="btn">
            ...
            <!-- //bttn selesai hidden  -->
            <button type="button" class="btn-selesai" style="display: none;">
                Selesai
            </button>
        </div>
        <!-- // -->
    ...
    <script>
        ...
        //button mulai waktu bergerak dan tampilkan button selesai
        $(document).ready(function() {
            $('.btn-mulai').click(function() {
                $('.btn-mulai').hide();
                var durasi = 90; // Durasi dalam menit
                // Hitung mundur durasi dari menit ke 0
                var countDownDate = new Date().getTime() + durasi * 60000;
                var x = setInterval(function() {
                    ...
                    document.getElementById("durasi").innerHTML = hours + "h " + minutes + "m " + seconds + "s ";
                    if (distance < 0) {
                        clearInterval(x);
                ...
                // Tampilkan tombol .btn-selesai
                $('.btn-selesai').show();
            });
        });
        $(document).ready(function() {
            //button selesai onclick alert selesai
            $('.btn-selesai').click(function() {
                var id_pemesanan = '{{ $pemesanan->id }}';
                //ajax
                $.ajax({
                    // riwayat/dijadwalkan/detail/{id}
                    url: '/terapis/riwayat/dijadwalkan/detail/' + id_pemesanan,
                    type: 'POST',
                    data: {
                    ...
                    success: function(response) {
                        if (response.status == 'success') {
                            ...
                            setTimeout(function() {
                                window.location.href = '/terapis/riwayat/selesai';
                            }, 2000);
                        } else {
